# -*- coding: utf-8 -*-
"""Plot the effect of hardcoded regional setpoint offsets."""

from __future__ import annotations

import argparse
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from matplotlib.lines import Line2D

from examples.chile.analyze_wood_stove_regional_dispersion import REGION_NAMES


GEOGRAPHIC_REGION_ORDER = [15, 1, 2, 3, 4, 5, 13, 6, 7, 16, 8, 9, 14, 10, 11, 12]


def _parse_args():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--without-offsets-dir", type=Path, required=True)
    parser.add_argument("--with-offsets-dir", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    return parser.parse_args()


def _regional_mean(path):
    frame = pd.read_csv(path / "simulation_results.csv")
    rows = []
    for code, group in frame.groupby("codigo_region", sort=False):
        rows.append(
            {
                "codigo_region": int(code),
                "region": REGION_NAMES[int(code)],
                "consumo": float(
                    np.average(group["wood_m3st"], weights=group["n_inmuebles"])
                ),
            }
        )
    return pd.DataFrame(rows)


def main():
    args = _parse_args()
    without = _regional_mean(args.without_offsets_dir).rename(
        columns={"consumo": "sin_offsets"}
    )
    with_offsets = _regional_mean(args.with_offsets_dir).rename(
        columns={"consumo": "con_offsets"}
    )
    redpe = pd.read_csv(args.with_offsets_dir / "regional_consumption_comparison.csv")
    redpe = redpe[
        ["codigo_region", "redpe_low_m3st", "redpe_mid_m3st", "redpe_high_m3st"]
    ]
    comparison = without.merge(
        with_offsets, on=["codigo_region", "region"], validate="one_to_one"
    ).merge(redpe, on="codigo_region", how="left", validate="one_to_one")
    comparison["change_pct"] = (
        comparison["con_offsets"] / comparison["sin_offsets"] - 1.0
    ) * 100.0
    ordered = (
        comparison.set_index("codigo_region")
        .reindex(GEOGRAPHIC_REGION_ORDER)
        .dropna(subset=["region"])
        .reset_index()
    )

    x = np.arange(len(ordered))
    width = 0.36
    labels = ordered["region"].tolist()
    fig, (ax_consumption, ax_change) = plt.subplots(
        2,
        1,
        figsize=(18, 11),
        sharex=True,
        gridspec_kw={"height_ratios": [2.1, 1]},
        constrained_layout=True,
    )
    ax_consumption.bar(
        x - width / 2,
        ordered["sin_offsets"],
        width,
        color="#8338ec",
        alpha=0.86,
        label="HDD14 ampliado, sin offsets regionales",
    )
    ax_consumption.bar(
        x + width / 2,
        ordered["con_offsets"],
        width,
        color="#e76f51",
        alpha=0.9,
        label="Con offsets de setpoint hardcodeados",
    )
    for xpos, row in zip(x, ordered.itertuples(index=False)):
        if np.isfinite(row.redpe_low_m3st):
            ax_consumption.vlines(
                xpos,
                row.redpe_low_m3st,
                row.redpe_high_m3st,
                color="#d62828",
                linewidth=5,
                alpha=0.85,
                zorder=5,
            )
            ax_consumption.scatter(
                xpos,
                row.redpe_mid_m3st,
                color="#8b0000",
                marker="D",
                s=42,
                zorder=6,
            )
    ax_consumption.set_ylabel("Consumo anual [m³ estéreo/año]")
    ax_consumption.set_title(
        "Efecto de offsets de setpoint regionales hardcodeados\n"
        "500 edificios · HDD14 ampliado · disparador exterior 10 °C"
    )
    ax_consumption.grid(axis="y", alpha=0.25)
    ax_consumption.legend(
        handles=[
            Line2D([0], [0], color="#8338ec", marker="s", markersize=10, linewidth=8, label="Sin offsets regionales"),
            Line2D([0], [0], color="#e76f51", marker="s", markersize=10, linewidth=8, label="Con offsets regionales"),
            Line2D([0], [0], color="#d62828", linewidth=5, label="REDPE: rango bajo–alto"),
            Line2D([0], [0], color="#8b0000", marker="D", linestyle="None", label="REDPE: punto medio"),
        ],
        loc="upper left",
        frameon=True,
    )

    ax_change.bar(
        x,
        ordered["change_pct"],
        color=np.where(ordered["change_pct"] >= 0, "#e76f51", "#43aa8b"),
        alpha=0.9,
    )
    ax_change.axhline(0, color="#222222", linewidth=1)
    ax_change.set_ylabel("Cambio [%]")
    ax_change.set_xlabel("Región")
    ax_change.set_xticks(x, labels, rotation=45, ha="right")
    ax_change.grid(axis="y", alpha=0.25)

    args.output.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(args.output, dpi=170)
    plt.close(fig)
    comparison.to_csv(args.output.with_suffix(".csv"), index=False)


if __name__ == "__main__":
    main()
